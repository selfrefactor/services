...
function applyCopyTrimmed(textEditor, startLine, endLine) {
  const lines = []
  ...
  if (lines.length === 0) {
    return logToUser('empty selection!!')
  }
  if (lines.length === 1) {
    copy(lines[0].trim())
  ...
  const withFirst = update(0, lines[0].trimLeft(), lines)
  const withLast = update(lines.length - 1, last(lines).trimRight(), withFirst)
  copy(withLast.join('\n'))
}
...
function copyTrimmedFn() {
  ...
  applyCopyTrimmed(textEditor, selection.start.line, selection.end.line)
