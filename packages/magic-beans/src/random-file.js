...
function sortFunction(a, b) {
	const aDirectories = a.split('/');
	const bDirectories = b.split('/');

	const directoryDepthA = a.split('/').length;
	const directoryDepthB = b.split('/').length;
	if (directoryDepthA < directoryDepthB) return -1;
	if (directoryDepthA > directoryDepthB) return 1;
	const fileNameA = last(aDirectories);
	const fileNameB = last(bDirectories);
	if (equals(aDirectories, bDirectories)) {
		return fileNameA > fileNameB ? -1 : 1;
	}
	const folderA = head(takeLast(2, aDirectories));
	const folderB = head(takeLast(2, bDirectories));
	if (!folderA && !folderB) {
		return fileNameA > fileNameB ? -1 : 1;
	}
	if (!folderA) return 1;
...
async function randomFileInitialize(
	...
	try {
		...
		const allFiles = getAllFiles(files, sequential);
		setter('files', allFiles);
	} catch (err) {
		console.log(err);
	}
}
...
exports.requestRandomFileWithSubfolderRightClickSequential =
	requestRandomFileWithSubfolderRightClick(true);
